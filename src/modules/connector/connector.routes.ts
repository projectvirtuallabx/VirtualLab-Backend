...
import {
  ...
  requireConnectorBearer
} from "../../middleware/auth.js";
import { applyConnectorCallback } from "../../services/connector-result.service.js";

const router = Router();

let taskQueue: Record<string, unknown>[] = [];

export const addTask = (task: Record<string, unknown>) => {
  taskQueue.push(task);
};

const RDP_TASK_TYPES = ["GENERATE_RDP", "BOOKING_CREATE"] as const;
...
async function claimNextDbTask(connectorId: string) {
